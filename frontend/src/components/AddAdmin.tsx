...
function AddAdmin({onClick} : {onClick:()=>void}) {
        ...
        <div className="z-20 fixed top-0 left-0 backdrop-blur w-full h-screen flex flex-col items-center justify-center">
            ...
            <div className={"shadow-2xl p-8 bg-white"}>
                <div className={"flex items-center justify-center"}>
                    <p className={"text-xl font-semibold"}>Add Admin</p>
                </div>
                <Input label={"name"} placeholder={"Admin Name"} onChange={(e) => {
                    setName(e.target.value);
                }}/>
                <Input label={"user-name"} placeholder={"Admin user-name"} onChange={(e) => {
